...
namespace AutoPost
{
    ...
    public partial class frmMain : Form
    {
        ...
        //    try
        //    {
        //        string messg = "result = 0;  message: パラメータが不正です";
        //        string[] jhsFiles = null;
        //        string[] jhsFiles_delete = null;              
        //        int delay = Convert.ToInt32(ConfigurationManager.AppSettings["Delay"]); // get all type file.jhs
        //        jhsFiles = Directory.GetFiles(path, extension);
        //        var result = "0";
        //        string propertyValue = "";
        //        List<string> listjhsdelete = new List<string>();
        //        Stream stream;
        ...
        //            if (_delay<delay)
        //            {
        //                listjhsdelete.Add(file);
        //            }
        //        }

        //        tmPostTimer.Stop();
        //        jhsFiles_delete = listjhsdelete.ToArray();

        //        foreach (string file in jhsFiles_delete)
        //        {
        //                String fileName = Path.GetFileName(file);      // get file name                                
        ...
        private void tmPostTimer_Tick(object sender, EventArgs e)
        {
            ...
            {
                logger.LogError("Wrong path!");
                //  WriteLog("Wrong path!");
                WriteLog_lk("Wrong path!");
            }
        }
        ...
        public void CallToChildThread(string file)
        {
                    ...
                    var result = "0";
                    string messg = "result = 0;  message: パラメータが不正です";
                    string data_1 = "status = 0;  message: 患者が存在しません";
                    string propertyValue = "";
            ...
            if (_delay < delay)
            {
                try  // Upload to Server.    
                {
                    ...
                    string resultJson = Encoding.ASCII.GetString(UploadFiles(webRoot, files, values));
                ...
                catch (Exception ex)
            {
                ...
                //logger.LogError("Unable to connect to the remote server");
                logger.LogException(ex);
            }
        }
        }
        ...
        private void frmMain_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                if (MessageBox.Show("自動アップロードを終了する. よろしいですか？", "終了を確認します", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == System.Windows.Forms.DialogResult.Cancel)
                    e.Cancel = true;
            }
        }
        ...
        public void WriteLog_lk(string message)
        {
            try
            {
